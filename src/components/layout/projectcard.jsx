    ...
    return (
        <div className="w-full px-4 py-3 rounded-xl bg-gray-100 dark:bg-gray-800 shadow-secondary-light dark:shadow-secondary-dark transition-shadow transition-colors duration-300">
            <div className="flex flex-col md:flex-row gap-3">
                {/* IMAGE */}
                <img className="w-full md:w-2/5 md:flex-shrink-0 aspect-[350/185] mb-auto lg:my-auto rounded-xl" src={project.image}/>
                {/* INFO */}
                    ...
                    <div className="flex justify-between content-center gap-1">
                        <div className="w-6/10 lg:w-4/5 flex gap-3 flex-wrap lg:flex-nowrap">
                            {project.tags.map((tagName, index) => (
                                <div key={index} className="px-2 py-1 text-center rounded-[7px] bg-blue-200 dark:bg-blue-950 transition-colors duration-300"><p className="font-light">{tagName}</p></div>
                            ))}
                        </div>
